...
<html lang="en">
  <head>
    ...
    <?php
      ...
      // get the subject id passed from the $_GET url
      $id = $_GET['update'];
      // get the record that matches the id or throw an error
      $record = $mysqli->query("SELECT * FROM scp WHERE id=$id")
        or die($mysqli->error());
      // get the subject as an array
      $subject = $record->fetch_assoc();
    ?>
  </head>
  <body>
    <!-- Navbar header -->
    ...
    <div class="container d-flex align-items-center justify-content-center h-100">
      <div style="width: 1000px; background-color: white; padding: 25px; border-radius: 25px;">
        ...
        <br>
        <input class="form-control" type="text" name="item" value="<?php echo $subject['item']; ?>">
        <br>
        <label>Object class:</label>
        <br>
        <input class="form-control" type="text" name="object_class" value="<?php echo $subject['object_class']; ?>">
        <br>
        <label>Subject image (optional):</label>
        <br>
        <input class="form-control" type="text" name="subject_image" value="<?php echo $subject['subject_image']; ?>">
        <br>
        <label>SCP Containment:</label>
        <br>
        <textarea class="form-control" name="containment" rows=5><?php echo $subject['containment']; ?></textarea>
        <br>
        <label>SCP Description:</label>
        <br>
        <textarea class="form-control" name="description" rows=5><?php echo $subject['description']; ?></textarea>
        <br>
        <input class="btn btn-warning" type="submit" name="update" value="Update Record">
        </form>
        <div class="my-3">
